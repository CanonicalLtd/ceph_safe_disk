...
pub struct Pools {
    ...
    pub min_size: i32,
    pub cache_target_dirty_high_ratio_micro: i32,
...
mod tests {
    ...
    fn osdmap_from_jewel_file_no_osd_panic() {
        ...
        assert_eq!(osdmap.osds.len() as i32, 3);
    }

    #[test]
    fn osdmap_from_jewel_file_no_osd() {
        let osdmap = OsdMap::from_file("test/jewel/osd_dump_no_osd.json").unwrap();
        // Default for ceph is min_size + 1 for replicated size
        assert_eq!(osdmap.pools.first().unwrap().size, 3);
    }
